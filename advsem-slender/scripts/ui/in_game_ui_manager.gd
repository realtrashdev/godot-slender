...
func _ready() -> void:
	...
	change_visible_characters(pages_text, pages_text.get_total_character_count(), 1)
	await get_tree().create_timer(3).timeout
	change_visible_characters(pages_text, 0, 1)
...
func change_visible_characters(text: RichTextLabel, visible_chars: int, time: float):
	...
	tween.tween_property(text, "visible_characters", visible_chars, time)
